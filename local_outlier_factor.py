...
# 2. Denormalisoidaan aidot X-muuttujat
with open('scalers.pkl', 'rb') as f:
    scalers = pickle.load(f)

# 3. Etsitään yhteiset X-muuttujat
...
# 4. Denormalisoidaan aidon datan featuret
...
# 5. Koodataan 'status_label' numeeriseksi molemmissa dataseteissä
le = LabelEncoder()
...
# 7. Valitaan vain yhteiset sarakkeet molemmista
X_real = real_denorm[lof_features]
X_synth = synthetic[lof_features]

# 8. Sovitetaan Local Outlier Factor aidolle datalle
lof = LocalOutlierFactor(n_neighbors=20, novelty=True)
lof.fit(X_real)

# 9. Lasketaan LOF-score generoidulle datalle
...
# Piirretään vertaileva histogrammi
